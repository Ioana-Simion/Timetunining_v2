...
from models import FeatureExtractor, FeatureForwarder, CorrespondenceDetection
...
class TimeTuningV2(torch.nn.Module):
    ...
    def correspondence_loss(self, crop_list, bboxs, img_size, dataset_features, dataset_q):
        ...
        for i in range(len(crop_list)):
            ...
            bbox = bboxs[:, i].long()
            cropped_labels_list = []
            for k in range(crops.size(1) -1):
                if k % 2 == 1:
                    continue
                img1_features = dataset_features[:, k, :, :]
                img2_features = dataset_features[:, k+1, :, :]
...
class TimeTuningV2Trainer():
    def __init__(self, data_module, test_dataloader, time_tuning_model, num_epochs, device, logger):
        ...
        self.logger = logger
        self.logger.watch(time_tuning_model, log="all", log_freq=30)
    ...
    def train_one_epoch(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            total_loss = 0 * clustering_loss + correspondece_loss
