...
from models import CrossAttentionBlock, FeatureExtractor, DistributedDataParallelModel
...
class PatchPredictionModel(torch.nn.Module):
    def __init__(self, input_size, vit_model, num_prototypes=200, prediction_window_size=2, masking_ratio=0.8, crop_size=96, logger=None, world_size=1):
    ...
    def forward(self, imgs1, imgs2, crop_list, bboxs):  
        self.normalize_prototypes()
        bs = imgs1.shape[0]
        # crop = random_crop_mask(imgs1)
        img1_features, img2_features = self.forward_backbone(imgs1, imgs2)
        pred_loss = 0
        clustering_loss = 0
        for i in range(len(crop_list)):
            ...
            q = find_optimal_assignment(scores, 0.05, 3, self.world_size)
        ...
        return pred_loss / len(crop_list), clustering_loss / len(crop_list)
        
    def train_step(self, imgs1, imgs2, crop_list, bboxs):
        pred_loss, clustering_loss = self.forward(imgs1, imgs2, crop_list, bboxs)
        return pred_loss, clustering_loss
    ...
    def validate_step1(self, img):
        ...
        with torch.no_grad():
            ...
            img1_features, img2_features = self.forward_backbone(img, img)
...
class PatchPredictionTrainer():
    def __init__(self, data_module, test_dataloader, patch_prediction_model, num_epochs, device, logger, world_size=1, rank=0):
        self.dataloader = data_module.data_loader
        if world_size > 1:
            self.sampler = data_module.sampler
    ...
    def setup_optimizer(self, optimization_config):
        ...
        max_itr = (self.num_epochs * num_itr) // self.world_size
    ...
    def train_one_epoch(self, epoch=0):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            loss, clustering_loss = self.patch_prediction_model.train_step(imgs1, imgs2, batch_crop_list[1:], label["bbox"][:, 1:, ])
    ...
    def validate(self, epoch, val_spatial_resolution=56):
        if isinstance(self.patch_prediction_model, DistributedDataParallelModel):
            print("DDP model")
            eval_model = self.patch_prediction_model.get_non_ddp_model()
        else:
            eval_model = self.patch_prediction_model
        eval_model.eval()
        with torch.no_grad():
            ...
            feature_spatial_resolution = eval_model.feature_extractor.eval_spatial_resolution
            feature_group = []
            targets = []
            for i, (x, y) in enumerate(self.test_dataloader):
                target = (y * 255).long()
                img = x.to(self.device)
                spatial_features = eval_model.validate_step(img)  # (B, np, dim)
...
def run(gpu=0, args=None):
    ...
    patch_prediction_model = PatchPredictionModel(224, vit_model, masking_ratio=masking_ratio, crop_size=crop_size, logger=logger, world_size=world_size)
    if world_size > 1:
        # patch_prediction_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(patch_prediction_model)
        patch_prediction_model = patch_prediction_model.to(device)
        patch_prediction_model = DistributedDataParallelModel(patch_prediction_model, gpu=gpu)
    ...
    patch_prediction_trainer = PatchPredictionTrainer(video_data_module, test_dataloader, patch_prediction_model, num_epochs, device, logger, world_size, rank)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--crop_augmentation', type=bool, default=True)
    parser.add_argument('--regular_step', type=int, default=2)
    parser.add_argument('--sampling_type', type=str, default="dense")
    ...
    parser.add_argument("--explaination", type=str, default="clustering, every other thing is the same; except the crop and reference are not of the same frame. and num_crops =4, crop_augmentation=True, inverse x-y, frame_distance 2")
